...
export function buildPriceQuery(item) {
  ...
  // is used. Doesn't apply to consoles, Funko Pops, or VHS — those are
  // always physical (there's no such thing as a digital VHS tape, unlike
  // DVD/CD/Book/Vinyl, which genuinely have digital editions — VHS joined
  // this list Sep 2026, having been silently left off since it shipped;
  // see components/GameModal.jsx's matching fix), and the Copy field
  // doesn't even show for any of the three, but the underlying copy_type
  // can briefly still hold a stale 'digital' value from before the item's
  // Type was switched (cleared for real on save).
  const alwaysPhysical = item.item_type === 'console' || item.item_type === 'funko_pop' || item.item_type === 'vhs';
