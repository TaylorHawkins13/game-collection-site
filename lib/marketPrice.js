...
export function buildPriceQuery(item) {
  ...
  // is used. Doesn't apply to consoles — those are always physical
  // hardware, and the Copy field doesn't even show for that type, but the
  // underlying copy_type can briefly still hold a stale 'digital' value
  // from before the item's Type was switched (cleared for real on save).
  if (item.copy_type === 'digital' && item.item_type !== 'console') return '';
