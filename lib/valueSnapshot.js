...
// items aren't part of "what your collection is worth." Digital items
// are excluded entirely (not just from the eBay side) — there's no
// resale market for them, so even a manually-entered purchase price
// shouldn't count toward collection value.
export function estimateCollectionValue(games) {
  const owned = (games || []).filter((g) => g.ownership === 'owned' && g.copy_type !== 'digital');
  let total = 0;
  let pricedCount = 0;
