...
// routes, see CHANGELOG.md): none of this file's three fetch calls had
// anything capping how long a slow Comic Vine response could hang the
// request, so a slow patch on their end could stall a route until
// Vercel's platform-level 300s ceiling killed it. Every fetch below now
// goes through fetchWithTimeout instead of the raw global — see the
// callers (app/api/comic-search/route.js, app/api/comic-detail/route.js)
// for the matching maxDuration backstop.
...
export async function searchComicVine(query) {
  ...
  try {
    ...
    const res = await fetchWithTimeout(`https://comicvine.gamespot.com/api/search/?${params.toString()}`, {
...
export async function getComicVineIssueDetail(issueId) {
  ...
  try {
    ...
    const issueRes = await fetchWithTimeout(`https://comicvine.gamespot.com/api/issue/4000-${id}/?${issueParams.toString()}`, {
      headers: { 'User-Agent': USER_AGENT, Accept: 'application/json' },
    });
    ...
    if (volumeId) {
      ...
      const volRes = await fetchWithTimeout(`https://comicvine.gamespot.com/api/volume/4050-${volumeId}/?${volParams.toString()}`, {
        headers: { 'User-Agent': USER_AGENT, Accept: 'application/json' },
      });
      if (volRes.ok) {
