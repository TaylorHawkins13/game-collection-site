...
// The inverse lookup — used server-side (app/api/ebay-price/route.js) to
// know which currency a search on a given marketplace should actually
// come back in, so listings priced in some other currency (a cross-border
// seller's item turning up on a search of a different country's site) can
// be filtered out rather than silently blended in.
const MARKETPLACE_TO_CURRENCY = Object.fromEntries(
  Object.entries(CURRENCY_TO_MARKETPLACE).map(([currency, marketplace]) => [marketplace, currency])
);

export function currencyForMarketplace(marketplace) {
  return MARKETPLACE_TO_CURRENCY[marketplace] || 'USD';
}
