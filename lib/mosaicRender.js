...
// Packs category-grouped rows (each already capped at perRowCap items,
// per lib/mosaicData.js's shapeMosaic) into fixed-width display rows that
// flow continuously across category boundaries, instead of giving every
// category its own full-width shelf regardless of how few items it has.
// A collector with 2 comics and 3 records used to get two nearly-empty
// full-width rows; now they share a shelf with whatever comes next,
// which is both denser and closer to how a real shelf actually looks —
// stuff doesn't stop just because the category changed. Each display row
// tracks which category label(s) it contains for the plank tag.
function packRows(categoryRows, perRow) {
  const flat = [];
  categoryRows.forEach((cr) => {
    cr.items.forEach((item) => flat.push({ item, categoryLabel: cr.label }));
    if (cr.overflow > 0) flat.push({ overflowCount: cr.overflow, categoryLabel: cr.label });
  });
  const packed = [];
  for (let i = 0; i < flat.length; i += perRow) {
    const chunk = flat.slice(i, i + perRow);
    const labels = [...new Set(chunk.map((c) => c.categoryLabel))];
    const label = labels.length > 2 ? 'Mixed Shelf' : labels.join(' + ');
    packed.push({ chunk, label });
  }
  return packed;
}

function ShelfRow({ displayRow, topValueIds, showcaseIds, currency }) {
  return h(
    'div',
    ...
    h(
      ...
      ...displayRow.chunk.map((entry, idx) =>
        entry.overflowCount != null
          ? h(OverflowTile, { key: `of-${idx}`, count: entry.overflowCount })
          : h(Tile, {
              key: entry.item.id,
              item: entry.item,
              cover: entry.item._resolvedCover,
              isShowcase: showcaseIds.has(entry.item.id),
              isTopValue: topValueIds.has(entry.item.id),
              currency,
            })
      )
    ),
    ...
    h(
      ...
      h(
        ...
        `${displayRow.label} · ${displayRow.chunk.length}`
      )
    )
  );
}

function renderShelfMosaicElement({ rows, username, totalItems, shownItems, modeLbl, topValueIds, showcaseIds, currency }) {
  const displayRows = packRows(rows, 10);
  const height =
    TOP_PAD +
  ...
  return {
    ...
    element: h(
      ...
      h(
        ...
        ...displayRows.map((displayRow, i) => h(ShelfRow, { key: i, displayRow, topValueIds, showcaseIds, currency })),
