...
// Excludes DLC/expansions/bundles/etc. from getFranchiseGames() below
// (Sep 2026 — Taylor: "stop it from showing irrelevant things like
// dlcs for games", then chose to "also exclude expansions/standalone
// expansions" when asked how aggressive to be). A DENYLIST, deliberately
// not an allowlist of category === 0 (main_game) — lib/igdbPlatformCatalogue.js
// already tried that exact allowlist approach for the platform-catalogue
// feature and had to remove it (see CHANGELOG.md): real production IGDB
// data showed plenty of legitimate main games with no `category` value
// set at all, not `0` — genuinely absent, not falsy-but-present. An
// allowlist would have silently hidden those. This only excludes rows
// explicitly tagged as one of the categories below; anything untagged
// (undefined/null `category`, which is common and normal) is kept by
// default, same as category 0.
//
// IGDB's full category enum, for reference: 0=main_game, 1=dlc_addon,
// 2=expansion, 3=bundle, 4=standalone_expansion, 5=mod, 6=episode,
// 7=season, 8=remake, 9=remaster, 10=expanded_game, 11=port, 12=fork,
// 13=pack, 14=update. Kept: 0 (main_game), 8/9/10 (remake/remaster/
// expanded — still a full game people collect), 11/12 (port/fork —
// same game on a different platform), and anything untagged. Excluded:
// 1/2/4 (DLC/expansion/standalone expansion — Taylor's "more
// aggressive" choice), 3 (bundle — not a distinct game), 5 (mod), 6/7
// (episode/season — fragments of a game, not standalone releases), 13
// (pack), 14 (update).
const DLC_CATEGORY_DENYLIST = new Set([1, 2, 3, 4, 5, 6, 7, 13, 14]);
  ...
  const franchiseLeg = (async () => {
    ...
    if (franchise) {
      const franchiseRes = await fetchWithTimeout('https://api.igdb.com/v4/franchises', {
        method: 'POST',
        headers,
        body: `fields games.name, games.cover.image_id, games.first_release_date, games.category; where id = ${franchise.id}; limit 500;`,
      });
      if (franchiseRes.ok) {
    ...
    const collectionResults = await Promise.all(
      gameCollections.map(async (c) => {
        const res = await fetchWithTimeout('https://api.igdb.com/v4/collections', {
          method: 'POST',
          headers,
          body: `fields games.name, games.cover.image_id, games.first_release_date, games.category; where id = ${c.id}; limit 500;`,
        });
        if (!res.ok) return [];
...
export async function getFranchiseGames(title) {
  ...
  try {
    ...
    const mapped = [...franchiseGamesRaw, ...collectionGamesRaw]
      .filter((g) => g.name && !DLC_CATEGORY_DENYLIST.has(g.category))
      .map((g) => ({
        id: g.id,
        name: g.name,
