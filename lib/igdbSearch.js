  ...
  try {
    ...
    const mapped = [...franchiseGamesRaw, ...collectionGamesRaw]
      ...
      .map((g) => ({
        ...
        // Sep 2026 for the Pull List feature (app/api/cron/refresh-
        // upcoming-releases), which needs a real date to sort/group a
        // calendar by, not just a year. `year` is left as-is since the
