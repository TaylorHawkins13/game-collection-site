...
// Was `$` only (see ROADMAP.md's former "Quick Add only recognizes $"
// item, fixed Sept 2026) — a GBP/EUR/etc. collector typing "got Fifa 06
// for £8 today" or "12,99€" got no price extracted at all. Every symbol
// in lib/currency.js's CURRENCIES list is recognized now, in either the
// prefix position most currencies here use ("$40", "£12.99") or the
// suffix position a few conventionally use instead ("12,99€", "40kr").
// Deliberately does NOT try to figure out *which* currency was typed —
// there's no currency field on a quick-add price the way there's no
// currency field on `price` generally (see the still-open ROADMAP.md
// item about `price`/`asking_price`/`price_alert_threshold` having no
// stored currency), so the number just prefills the Add form's Purchase
// price field, already understood to be in the collector's own profile
// currency, same as typing it in by hand always has been. That sidesteps
// the genuinely hard part of currency detection (JPY and CNY share the
// exact same ¥ symbol, for instance — nothing in a quick-add sentence
// could disambiguate that) without losing the point of this fix: today,
// the failure mode was "no price captured at all," not "captured under
// the wrong currency label."
//
// The matched number's own decimal separator can be either '.' or ','
// (US/UK "12.99" vs. many European currencies' "12,99" — normalized to
// '.' before parseFloat below). Deliberately doesn't also try to parse
// thousands-grouping in the same pattern (a typed "$1,234.56" for one
// collectible via a casual quick-add sentence is vanishingly rare, and
// supporting it would reintroduce exactly the "is this comma a decimal
// or a thousands separator" ambiguity this fix is trying to avoid).
//
// A few symbols (R for Rand, kr for Krona, CHF) are bare letters rather
// than currency-only punctuation, so they're bounded against an adjacent
// *letter* — `(?<![A-Za-z])...(?![A-Za-z])` — to avoid matching inside an
// unrelated word like "Trigger" or "Raider". Deliberately NOT a regular
// `\b` word boundary: `\b` doesn't fire between a digit and a letter
// (both count as "word" characters to it), which would break the exact
// no-space suffix form these symbols are meant to catch ("150kr"). A
// digit sitting directly against the symbol, with no letter on the outer
// side, is exactly the case that should match. Since a wrong price match
// here is just a form field to notice and correct before Save, never a
// silent bad save (see this file's own module comment), a rare false
// positive is a minor annoyance, not a data-integrity risk.
...
const PRICE_RE = new RegExp(
  `(?:${CURRENCY_SYMBOLS})\\s?(\\d+(?:[.,]\\d{1,2})?)|(\\d+(?:[.,]\\d{1,2})?)\\s?(?:${CURRENCY_SYMBOLS})`,
  'i'
);
...
export function parseQuickAddText(raw) {
  ...
  if (priceMatch) {
    // Whichever branch of PRICE_RE matched — symbol-first ("$40") or
    // symbol-last ("40kr") — puts its digits in a different capture group;
    // exactly one of the two is ever set. The decimal separator is
    // normalized to '.' before parsing, since a matched "12,99" means the
    // same 12.99 a "12.99" match would.
