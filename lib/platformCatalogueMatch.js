...
// platforms + ownership — see app/dashboard/catalogue/page.js). `ownedPlatformIds`
...
// directly alongside the identical bug in lib/seriesLookup.js's
// ownedKeysFor: a wishlisted GameCube title was showing as owned/logged
// here too, since this never checked `ownership` either — any row with a
// matching title/platform counted, wishlist and sold included. Same "only
// ownership === 'owned' counts" rule as everywhere else in the app.
export function ownedTitleKeysForPlatform(ownedGames, ownedPlatformIds, targetPlatformId, targetPlatformName) {
  ...
  for (const g of ownedGames || []) {
    if (!g?.title || g.ownership !== 'owned') continue;
