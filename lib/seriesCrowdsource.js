...
export { normalizeSeriesText };

// The "Series" feature's crowdsourced-fallback backend. Funko Pops and
// Vinyl/CD/DVD/VHS (Aug 2026 — see CHANGELOG.md) go straight here; comics
// go to Comic Vine, trading cards try TCGdex/Scryfall first, and books
// (Sep 2026) try Open Library first (see lib/useSeriesLookup.js) — all
// three fall back to this for anything their real backend doesn't
// recognize, trading cards in particular, since TCGdex is Pokémon-only
// and a non-Pokémon card (Magic, mainly) always misses there. Works off
// Shelf Life's own data instead of a real canonical database — every
// matching row anyone on the site has logged (RLS scopes `games` reads to
// public profiles, the viewer's own rows, and — a separate branch, see
// ROADMAP.md item 3 — wishlist rows on any profile with `wishlist_public`
// on). Deliberately not narrowed to `ownership: 'owned'` the way
// app/page.js's homepage rows and the Find Collectors search were (Sept
// 2026, see CHANGELOG.md): no username/identity is attached to anything
// this returns, just title/cover/set/number used to build a "does this
// variant exist" checklist, and a wishlist item is still real evidence
// that variant exists — unlike those two spots, there's no privacy
// exposure in including it here. Weaker than a real canonical list — it
// can only know about issues/cards/albums/movies/books someone's actually
...
// normalizeSeriesText (loose-match series-name normalization — strips
// punctuation/spacing/case differences so "Marvel Series 1" and "Marvel
// - Series 1" key the same) moved to lib/textNormalize.js (Sep 2026) so
// lib/upcomingReleases.js can reuse the exact same normalization without
// pulling in this file's own top-level `@/lib/supabaseServer` import —
// see that file's header comment for the full reasoning. Re-imported
// (and re-exported below) rather than defined locally, so every call
...
//
// Also runs the same best-effort English-only filter (Sep 2026 —
// Taylor, right after the Games/Books accuracy pass: "do it for other
// collectibles") that Books' real Open Library backend already got —
// `looksNonEnglish()` moved to lib/textNormalize.js specifically so it
// could be reused here unchanged. A foreign-language pressing/edition
// someone's logged (a French vinyl reissue, a German DVD release) is the
// exact same kind of noise a translated book edition was, just coming
// from Shelf Life's own crowdsourced data instead of Open Library.
async function getCrowdsourcedCreatorSeries(supabase, itemType, creatorCfg, value, normalizedTarget) {
  ...
  const rows = (data || []).filter(
    (row) => normalizeSeriesText(row[creatorCfg.creatorCol]) === normalizedTarget && !looksNonEnglish(row.title)
  );
