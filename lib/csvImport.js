...
export const ITEM_TYPES = ['game', 'comic', 'trading_card', 'vinyl', 'book', 'dvd', 'cd', 'console'];
...
const HAS_PUBLISHER = ['comic', 'trading_card', 'vinyl', 'book', 'dvd', 'cd', 'console'];
const HAS_WRITER = ['comic', 'book', 'dvd', 'cd'];
const HAS_ARTIST = ['comic', 'vinyl'];
const HAS_GRADE = ['comic', 'trading_card', 'console'];
const HAS_VARIANT = ['comic', 'trading_card'];
...
export function normalizeRow(raw) {
  ...
  // A console is always physical hardware — "digital" describes a game
  // with no physical copy at all, which doesn't apply to a console, so
  // any digital/physical value in the CSV is ignored for this type (same
  // rule GameModal's handleSave enforces when the type is switched there).
  if (item_type === 'console') copy_type = 'physical';
