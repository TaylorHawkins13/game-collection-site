...
// price-drop threshold" item flagged in ROADMAP.md): price_alert_threshold
// and wishlist_priority are both wishlist-only fields (see the gift-list
// priority feature in CHANGELOG.md) that never round-tripped through CSV
// import/export before. Both mirror GameModal's own rule — cleared to
// null unless ownership is 'wishlist' — and wishlist_priority is
// validated against the same 1/2/3 (High/Medium/Low) the database's check
// constraint enforces.

export const ITEM_TYPES = ['game', 'comic', 'trading_card', 'vinyl', 'book', 'dvd', 'vhs', 'cd', 'console', 'funko_pop'];
...
const HAS_PUBLISHER = ['comic', 'trading_card', 'vinyl', 'book', 'dvd', 'vhs', 'cd', 'console', 'funko_pop'];
const HAS_WRITER = ['comic', 'book', 'dvd', 'vhs', 'cd'];
...
const HAS_FORMAT = ['vinyl', 'book', 'dvd', 'vhs', 'cd', 'console'];
const HAS_REGION_COMPLETENESS = ['game', 'console'];
const HAS_CARD_FIELDS = ['trading_card', 'funko_pop'];
// VHS added Sep 2026 — found while auditing every "built for one type,
// silently extended to all" assumption after the CeX/Amazon buy-link
// fixes: unlike DVD/CD/Book/Vinyl, which genuinely have digital editions,
// there's no such thing as a digital VHS tape, but it had been left off
// this list since it shipped — see components/GameModal.jsx's matching
// fix for the full chain this fed into.
const ALWAYS_PHYSICAL = ['console', 'funko_pop', 'vhs'];
...
// wishlist_priority is a smallint check(1, 2, 3) at the database level
// (see wishlist-priority-migration.sql) — 1/2/3 only, same as the "High/
// Medium/Low" select in GameModal.jsx. Anything else in the CSV is
// dropped with a warning rather than sent through and rejected by the
// database check constraint.
function parseWishlistPriority(val) {
  const v = (val || '').trim();
  if (!v) return null;
...
export function normalizeRow(raw) {
  ...
  // Both wishlist-only, same rule GameModal's handleSave already enforces:
  // cleared to null unless ownership is actually 'wishlist', so switching
  // an item's ownership away from Wishlist can't leave a stale priority/
  // threshold behind (same as editing an existing item by hand).
  const priceAlertThreshold = ownership === 'wishlist' ? parsePrice(raw.price_alert_threshold) : null;
  if (ownership === 'wishlist' && raw.price_alert_threshold && raw.price_alert_threshold.trim() && priceAlertThreshold === null) {
    warnings.push(`price_alert_threshold "${raw.price_alert_threshold}" ignored — not a number`);
  }
  const wishlistPriority = ownership === 'wishlist' ? parseWishlistPriority(raw.wishlist_priority) : null;
  if (ownership === 'wishlist' && raw.wishlist_priority && raw.wishlist_priority.trim() && wishlistPriority === null) {
    warnings.push(`wishlist_priority "${raw.wishlist_priority}" ignored — must be 1 (High), 2 (Medium), or 3 (Low)`);
  }
