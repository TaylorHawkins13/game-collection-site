...
// labeled Artist/Director/Director/Author respectively). Vinyl/CD/DVD/
// VHS stay crowdsourced-only (lib/seriesCrowdsource.js) — no real
// canonical discography/filmography API is wired up for them. Books are
// the one exception (Sep 2026): a real per-author bibliography from Open
// Library now backs this instead — see lib/openLibraryAuthorLookup.js
// and lib/useSeriesLookup.js — with the crowdsourced path still there as
// a fallback for an author Open Library doesn't recognize, same pattern
// comics/trading cards already use for their own real backends.
...
// GameModal/ItemDetailModal. Funko Pops share comics'
...
// ItemDetailModal) and, via lib/platformCatalogueMatch.js's
...
// Owned-issue keys for the Comics "Creator page" feature (ROADMAP.md —
// see lib/comicVineCreatorLookup.js for the full reasoning). Deliberately
// separate from ownedKeysFor() above rather than an extra branch on it:
// ownedKeysFor's comic case keys on issue_number alone, which is only
// safe because every existing caller (GameModal/ItemDetailModal, via
// SeriesGrid) always compares within one already-chosen
// series — a creator page's grid spans every series a writer/artist has
// ever worked on at once, so keying on issue_number alone would collide
// across series (owning Detective Comics #1 would wrongly show as owning
// every other series' own issue #1 too). Keys on series *and* issue
// number together instead, using the exact same "series::number" format
// lib/comicVineCreatorLookup.js's creatorMatchKey() builds its entries'
// matchKey from, so the two sides actually line up. Only `ownership ===
// 'owned'` rows count, same rule ownedKeysFor already applies.
export function ownedComicKeysBySeriesAndIssue(items) {
  const set = new Set();
  for (const item of items || []) {
    if (item.item_type !== 'comic' || item.ownership !== 'owned') continue;
    if (!item.series || item.issue_number == null || item.issue_number === '') continue;
    set.add(`${normalizeTitle(item.series)}::${normalizeTitle(String(item.issue_number))}`);
  }
  return set;
}
