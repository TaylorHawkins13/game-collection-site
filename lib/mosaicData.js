...
// Per-type accent color + icon — the visual-identity system behind the
// dashboard's segmented shelf hero (components/ShelfIdentityHero.jsx, see
// ROADMAP.md/CHANGELOG.md) and anywhere else a collectible type wants a
// consistent color/icon of its own, not just a name. A hand-picked map
// rather than a hash function like titleColor() below — there are only
// ever exactly 10 known categories, so a fixed, spaced-out set of hues
// gives more control than hashing risks (two categories landing on
// near-identical hues, or an unpleasant one for a specific type). Fed
// into the same `--tile-color` CSS custom property the "Browse by
// system" tiles already use (app/globals.css's .system-tile rules), so
// hover/active states get the same color-mix() treatment for free.
...
// A short, letters-only monogram per type — the non-emoji replacement for
// what used to be an emoji per category (requested directly: "remove
// emojis from the site and app"). Two letters rather than one since a
// single initial collides for several types (Comic/CD/Console would all
// be "C"; Vinyl/VHS would both be "V") — kept short enough to read as a
// small badge rather than an abbreviation someone has to parse. Styled
// as a colored chip in ShelfIdentityHero.jsx using the same --tile-color
// custom property TYPE_COLORS already feeds elsewhere.
export const TYPE_MONOGRAMS = {
  game: 'GM',
  comic: 'CM',
  trading_card: 'TC',
  vinyl: 'VN',
...
// The collector-language word for "the place these live" per type — used
// to flavor empty-state copy (WelcomePanel.jsx, ProfileTabs.jsx) and
// trophy flavor text (TrophyCase.jsx) toward how an actual collector of
// that type would talk, instead of generic copy for every account. See
// ROADMAP.md "Type-aware microcopy and trophy-badge flavor." Several
// types genuinely share "shelf," which is also the untouched default
// wording used when nothing more specific is known — so those render
// identically to unflavored copy, on purpose, rather than forcing an
// invented synonym where none really exists.
...
// Singular form of one item of a type, for sentences like "every X you
// want to keep track of" — same flavoring use as TYPE_NOUNS above.
...
// The item_type with the most items in a list, ties broken by
// CATEGORY_ORDER (arbitrary but deterministic), null for an empty list.
// Powers the type-aware flavor text above without requiring someone to
// have narrowed their Collecting preferences down to exactly one type —
// works off whatever they've actually logged the most of.
export function dominantType(items) {
  const counts = {};
  (items || []).forEach((i) => {
    if (!i || !i.item_type) return;
    counts[i.item_type] = (counts[i.item_type] || 0) + 1;
  });
  let best = null;
  let bestCount = 0;
  CATEGORY_ORDER.forEach((t) => {
    const c = counts[t] || 0;
    if (c > bestCount) {
      best = t;
      bestCount = c;
