...
import { ebaySearchUrl, amazonSearchUrl } from './externalListings';
...
export function amazonBuyLink(item, currency) {
  ...
  const base = amazonSearchUrl(title, currency);
  const tag = process.env.NEXT_PUBLIC_AMAZON_ASSOCIATE_TAG;
  // Region-aware now (Sep 2026 — see lib/externalListings.js's
  // amazonSearchUrl/AMAZON_DOMAIN_BY_CURRENCY comment for why: this used
  // to always link to amazon.com regardless of the collector's own
  // currency). The associate tag deliberately only gets appended when the
  // link actually landed on amazon.com — Amazon Associates tags are
  // registered per marketplace, not global, so tacking a US tag onto an
  // amazon.co.uk/.de/.ca/.com.au URL wouldn't actually earn anything and
  // could read as a malformed/untrusted param on a storefront it was
  // never registered for. Every other region still gets a real, working
  // plain search link — same "these work untracked even with neither
  // program configured" tolerance this file's own header comment already
  // establishes, just extended to "untracked for regions with no
  // matching tag" too.
  const isAmazonCom = base.startsWith('https://www.amazon.com/');
  if (tag && isAmazonCom) {
