...
export async function getComicMasterSetEntries(seriesName) {
  ...
  // Comic Vine sometimes lists more than one issue object under the same
  // issue_number for a volume — a variant-cover printing or a reprint
  // catalogued as its own issue ID rather than folded into the original
  // (Sep 2026 — Taylor, after the Games/Books accuracy pass: "do it for
  // other collectibles"). Left unhandled, that shows as two tiles both
  // labeled "#1" in the grid, since normalizeSeriesResponse() below just
  // maps entries straight through with no dedup of its own. Collapsed to
  // one entry per normalized issue number, preferring whichever copy has
  // cover art — same normalizeCardNumber-keyed byNumber Map pattern
  // lib/seriesCrowdsource.js's getCrowdsourcedSeries() already uses for
  // the identical "same number, logged twice" problem on its own
  // crowdsourced path, reused here rather than reinvented, and consistent
  // with every other numbered-entry dedup in this codebase.
  const byNumber = new Map();
  for (const entry of mapped) {
    const key = normalizeCardNumber(entry.number);
    if (!key) continue;
