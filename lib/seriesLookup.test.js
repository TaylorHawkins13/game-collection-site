...
describe('seriesSupported / isMasterSetType', () => {
  ...
  it('supports Vinyl/CD/DVD/VHS/Book via the crowdsourced creator-based path (Aug 2026)', () => {
    expect(seriesSupported('vinyl')).toBe(true);
    expect(seriesSupported('cd')).toBe(true);
    expect(seriesSupported('dvd')).toBe(true);
    expect(seriesSupported('vhs')).toBe(true);
    expect(seriesSupported('book')).toBe(true);
  });

  it('does not support Consoles — no per-item "creator" field forms a completable series', () => {
  ...
  it('only trading cards and comics have a real master-set backend', () => {
    ...
    // The 5 new creator-based types stay on "See full series" copy too,
    // same as Funko Pops — crowdsourced, not a real canonical backend.
    expect(isMasterSetType('vinyl')).toBe(false);
    expect(isMasterSetType('book')).toBe(false);
  });
});
...
describe('seriesQueryValueFor', () => {
  ...
  it('uses the dedicated `artist` column for Vinyl', () => {
    expect(seriesQueryValueFor({ item_type: 'vinyl', artist: 'Radiohead' })).toBe('Radiohead');
  });

  it('uses the shared `writer` column (Director/Author/Artist per type) for CD/DVD/VHS/Book', () => {
    expect(seriesQueryValueFor({ item_type: 'dvd', writer: 'Christopher Nolan' })).toBe('Christopher Nolan');
    expect(seriesQueryValueFor({ item_type: 'book', writer: 'Brandon Sanderson' })).toBe('Brandon Sanderson');
  });

  it('returns an empty string for an unsupported type or missing item', () => {
    expect(seriesQueryValueFor({ item_type: 'console' })).toBe('');
...
describe('ownedKeysFor', () => {
  it('keys games by normalized title', () => {
    const items = [{ item_type: 'game', title: 'Chrono Trigger', ownership: 'owned' }];
  ...
  it('keys trading cards by normalized number + guessed print variant', () => {
    const items = [
      { item_type: 'trading_card', card_number: '4/102', is_variant: false, ownership: 'owned' },
      { item_type: 'trading_card', card_number: '7/102', is_variant: true, variant_notes: 'Reverse holo', ownership: 'owned' },
    ];
  ...
  it('guesses Magic-style foil/etched variants the same way as Pokémon-style ones', () => {
    ...
    const items = [
      { item_type: 'trading_card', card_number: '150', is_variant: true, variant_notes: 'Foil', ownership: 'owned' },
      { item_type: 'trading_card', card_number: '151', is_variant: true, variant_notes: 'Etched foil', ownership: 'owned' },
      { item_type: 'trading_card', card_number: '152', is_variant: false, ownership: 'owned' },
    ];
    const keys = ownedKeysFor(items, 'trading_card');
  ...
  it('keys comics by normalized issue number', () => {
    const items = [{ item_type: 'comic', issue_number: '12', ownership: 'owned' }];
  ...
  it('keys Vinyl/CD/DVD/VHS/Book by normalized title, not a number field', () => {
    const items = [
      { item_type: 'vinyl', artist: 'Radiohead', title: 'OK Computer', ownership: 'owned' },
      { item_type: 'book', writer: 'Brandon Sanderson', title: 'The Way of Kings', ownership: 'owned' },
    ];
    expect(ownedKeysFor(items, 'vinyl').has('ok computer')).toBe(true);
    expect(ownedKeysFor(items, 'book').has('the way of kings')).toBe(true);
  });

  // Bug fix (Sep 2026, flagged directly: "it says have pokemon xd even
  // though its just wishlisted") — a matching title/number used to count
  // regardless of ownership status; now only a real 'owned' row does.
  it('does not count a wishlisted or sold item as owned, even with a perfectly matching title', () => {
    const items = [
      { item_type: 'game', title: 'Pokemon XD: Gale of Darkness', ownership: 'wishlist' },
      { item_type: 'game', title: 'Sold Game', ownership: 'sold' },
    ];
    expect(ownedKeysFor(items, 'game').size).toBe(0);
  });
});
...
describe('variantHintsFor', () => {
  it('builds "number:variant" hints for owned variant copies within one card set', () => {
    const items = [
      { item_type: 'trading_card', card_set: 'Base Set', card_number: '4', is_variant: true, variant_notes: 'holo', ownership: 'owned' },
      { item_type: 'trading_card', card_set: 'Base Set', card_number: '7', is_variant: false, ownership: 'owned' }, // not a variant
      { item_type: 'trading_card', card_set: 'Jungle', card_number: '4', is_variant: true, variant_notes: 'holo', ownership: 'owned' }, // different set
    ];
  ...
  it('matches the card set case-insensitively', () => {
    const items = [
      { item_type: 'trading_card', card_set: 'base set', card_number: '4', is_variant: true, variant_notes: '1st edition', ownership: 'owned' },
    ];
    expect(variantHintsFor(items, 'Base Set')).toEqual(['4:firstEdition']);
  });

  it('returns nothing for a blank set value', () => {
    expect(variantHintsFor([{ item_type: 'trading_card', is_variant: true, ownership: 'owned' }], '')).toEqual([]);
  });

  // Same bug/fix as ownedKeysFor above — a wishlisted card's variant used
  // to get folded in as owned "evidence" right alongside a real one.
  it('does not hint at a wishlisted variant copy', () => {
    const items = [
      { item_type: 'trading_card', card_set: 'Base Set', card_number: '4', is_variant: true, variant_notes: 'holo', ownership: 'wishlist' },
    ];
    expect(variantHintsFor(items, 'Base Set')).toEqual([]);
  });
});
...
describe('normalizeSeriesResponse', () => {
  ...
  it('normalizes a title-keyed (Vinyl/CD/DVD/VHS/Book) response — no number field at all', () => {
    const json = { seriesName: 'Radiohead', entries: [{ id: 1, cover: 'x.jpg', title: 'OK Computer' }] };
    const result = normalizeSeriesResponse('vinyl', json);
    expect(result.entries[0]).toMatchObject({ label: 'OK Computer', number: null, matchKey: 'ok computer' });
  });
});
...
describe('prefillFromSeriesEntry', () => {
  ...
  it('builds a Vinyl prefill with the dedicated artist field, from title alone', () => {
    const entry = { rawTitle: 'OK Computer', label: 'OK Computer', cover: 'x.jpg' };
    expect(prefillFromSeriesEntry('vinyl', 'Radiohead', entry)).toMatchObject({
      item_type: 'vinyl',
      title: 'OK Computer',
      artist: 'Radiohead',
    });
  ...
  it('builds a Book prefill with the shared writer field', () => {
    const entry = { rawTitle: 'The Way of Kings', label: 'The Way of Kings', cover: 'x.jpg' };
    expect(prefillFromSeriesEntry('book', 'Brandon Sanderson', entry)).toMatchObject({
      item_type: 'book',
