...
// GameModal (editing your own item) and ItemDetailModal (read-only,
// viewing your own dashboard or anyone's public profile). Games hit
// /api/igdb-franchise; trading cards try
...
// books hit /api/book-master-set (a real per-author Open Library
// bibliography — see lib/openLibraryAuthorLookup.js). Funko Pops, Vinyl,
// CD, DVD, and VHS go straight to /api/series-lookup, the crowdsourced
// fallback used by anything that doesn't have a real canonical-data
// backend — see lib/seriesLookup.js for the full picture, and
// normalizeSeriesResponse for how the crowdsourced/IGDB/comic-master-set/
// book-master-set shapes all end up looking the same to the caller.
//
...
export default function useSeriesLookup() {
  ...
  const load = useCallback(async (itemType, value, variantNumbers) => {
    ...
    try {
      ...
      const isBookMasterSet = itemType === 'book';
      const variantParam =
      ...
      if (isTradingCard) {
        ...
        json = await res.json();
      } else if (isBookMasterSet) {
        const res = await fetch(`/api/book-master-set?value=${encodeURIComponent(v)}`);
        json = await res.json();
      } else {
      ...
      // TCGdex+Scryfall (trading cards), Comic Vine (comics), and Open
      // Library (books) all only cover what they cover. Any of them
      // coming back `no_series` falls through to the same crowdsourced
      // backend Funko Pops/Vinyl/CD/DVD/VHS use, instead of just
      // dead-ending here. See the module comment above.
      if (json.error === 'no_series' && (isTradingCard || isComicMasterSet || isBookMasterSet)) {
        ...
        setError(
          isTradingCard
            ...
            : "Couldn't find that author on Open Library, and nobody on Shelf Life has logged anything else by them yet either — this works best when the author field matches how Open Library spells their name (the book Search button fills this in for you)."
        );
        return;
      }
