...
- **Series/franchise completion view** — tap an item (e.g. any GTA game) and see the whole franchise laid out, with entries not yet in your collection greyed out, so it reads at a glance as "how close am I to the full set" instead of just a flat list. Genuinely good idea, but the real open question is where "everything in the series" comes from — Shelf Life has no canonical franchise database today. Two different answers depending on type:
  - **Games** — the cleanest starting point. IGDB (already used for game auto-fill) has real franchise/collection data, so "all GTA games" could be pulled live from IGDB whenever an item with a known franchise is opened — no new data to build or maintain.
  - **Comics / trading cards / vinyl** — no equivalent free public "franchise database" exists for these. Would have to work off what's already self-reported across the site (`series`/`card_set` + issue/card number), flagging gaps in numbers already logged rather than promising a true canonical total (can't know a run has 12 issues if nobody's ever logged past #8). Weaker, but buildable without a new integration.
  Realistic shape: ship it for games first since the data source is solid, treat the rest as a stretch goal.
