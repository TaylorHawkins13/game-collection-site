...
- **Series/franchise completion view — built for games, needs a live test** — editing an existing game item now has a "See full series" button in GameModal that pulls the whole IGDB franchise (e.g. every GTA game) and greys out anything not already in your collection. Built and compiles clean, but the actual IGDB network calls couldn't be exercised from this sandbox (no outbound access to Twitch/IGDB's domains here) — try it on a real game once this is pushed and live, and flag anything that looks off (wrong franchise picked, empty result on a game that should have one, etc.) so it can move to `CHANGELOG.md` once confirmed working.
  - **Comics / trading cards / vinyl** — not built. No equivalent free "franchise database" exists for these types, so it'd have to work off what's already self-reported across the site (`series`/`card_set` + issue/card number) and flag gaps in numbers already logged, rather than a true canonical total. Worth doing if the games version proves useful.
