...
- **Article submissions have no rate limit** — comments already cap at 5 per 5 minutes (server-enforced, since a fix a while back); `/api/articles/submit` has none. A submission can't actually go public without your approval in `/admin/articles`, so this isn't a content-safety hole, but a burst of junk submissions would flood that queue and spam your inbox (each one sends a notification email). Worth the same kind of limit comments already have.
- **RLS performance cleanup (not urgent)** — Supabase's advisor flagged roughly 25 row-level-security policies across most tables where `auth.uid()` gets re-evaluated per-row instead of once per query (the fix is wrapping it as `(select auth.uid())`), plus two duplicate SELECT policies stacking on `article_submissions`, and 2 indexes that have never actually been used. None of this is a correctness bug — everything still returns the right data — it's a query-planner efficiency thing that starts to matter at real scale, not at friends-and-family scale. Better as a deliberate pass through ~15 policies on a quiet day than something rushed right before launch.
- **Dashboard's JS bundle is the heaviest page on the site by a wide margin** — 158kB of page-specific JS (298kB first load) vs. ~2-6kB for everything else, noticed in the build output while checking for other issues. Not a correctness problem, but worth a code-splitting/lazy-load pass once things settle — it's the single page every signed-in visit hits.
