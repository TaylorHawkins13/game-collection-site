...
## Rejected by Apple, resubmitted — awaiting a response

First submission came back rejected; whatever was flagged has been addressed and the app's been **resubmitted for review**. Typical turnaround is roughly 24-48 hours, though it varies — you'll get an email the moment status changes (Waiting for Review → In Review → Ready for Sale, or another rejection with specifics). Nothing to do right now but wait.

If this round also comes back rejected, bring me the exact rejection message (which guideline number Apple cites, and their reviewer notes) so the fix is targeted at the real reason rather than a guess — the "If it gets rejected" section of `app-store-xcode-walkthrough.md` has the most likely candidates (Guideline 4.2 "repackaged website" chief among them) as a starting point.
...
- **Enable "Leaked password protection" in Supabase (2-minute manual step)** — Auth → Providers → Email has a toggle that checks new passwords against HaveIBeenPwned's breach database before accepting them. It's off right now, flagged by Supabase's own security advisor. Dashboard-only setting — there's no tool available here that can flip Auth config, only the database itself. Worth doing before wide public signups start.
...
- **Next.js has a known security vulnerability at the pinned version (14.2.5)** — `npm install` now warns about it directly ("This version has a security vulnerability. Please upgrade to a patched version"). Not something to silently `npm audit fix --force` right before/after launch without testing — worth a deliberate upgrade-and-retest pass. Flagged here rather than acted on.

## Requested by Taylor (not yet built)

- **Full series view: let a missing entry link straight to an eBay check** — the "See full series" grid greys out what you don't own, but there's no way to act on that from the grid itself; clicking a greyed-out entry should offer a way through to the same eBay price check the item form already has, so spotting a gap turns straight into "what would this cost me."
- **Collection value over time: chart text overlaps** — bug in the value-over-time chart (`ValueChart`, shown on the dashboard/insights) — labels are colliding rather than staying legible. Needs a look at spacing/collision handling, likely worse with a particular number of data points or longer currency-formatted labels.
- **Collection/profile cards: add an explicit Edit button, and make the default click open a clearer detail view** — right now clicking a card presumably jumps straight into editing; wants a real view/detail state as the default click target with Edit as its own separate, explicit action instead.
- **Collection/profile cards: some text gets cut off / doesn't fit** — market value is the specific example given (invisible on some items), but likely a broader overflow issue on the card layout, not just that one field. Worth a pass across `GameCard` checking what else silently truncates or overflows at typical card widths.
...
- **"What's new" entries should be clickable for a fuller breakdown** — right now it's just a teaser list; each entry should open up more detail on what actually shipped (could reuse/link into the relevant `CHANGELOG.md` entry, or a dedicated changelog page).
- **Trading card entry is too hard — needs a UX pass** — general pain point, no specific repro yet; worth digging into what's actually friction-y about adding a trading card (the Search auto-fill flow, the field layout, something else) before redesigning it blind. Bring specifics (which step trips you up) when there's time to dig in.
