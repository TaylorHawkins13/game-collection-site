...
- **Trading card scanner** — a "Scan Card" button (trading cards only) photographs the card and reads its name off it with on-device OCR, then Search looks it up on free Pokémon TCG and Magic: The Gathering databases to auto-fill set, card number, cover art, and publisher. Sports cards and other TCGs (Yu-Gi-Oh, etc.) aren't covered yet — no good free database exists for those — and OCR accuracy varies with lighting/glare, so results always need a human to confirm before applying.
