...
create table if not exists profiles (
  ...
  steam_id text,
  -- Set the moment self-service account deletion is requested
  -- (app/api/account/delete); the actual cleanup runs later, once
  -- GRACE_PERIOD_HOURS (lib/accountDeletion.js) has passed — see
  -- app/api/cron/process-account-deletions. Null means no deletion is
  -- pending. Signing back in during the window offers a way to clear
  -- this back to null instead.
...
-- A fixed, hand-maintained USD conversion table — NOT live rates. Exists
-- solely so the "most valuable" leaderboards below can rank collectors
-- fairly across different display currencies instead of comparing raw
-- numbers as if they were the same unit (a €5,000 collection used to
-- outrank a real $6,000 one just because 5,000 > 6,000 as plain
-- numbers). Deliberately not wired to a live rates API — that's real
-- currency conversion sitewide, a bigger project of its own (see
-- ROADMAP.md "Live currency conversion"). Update the values below by
-- hand occasionally; a ranking like this doesn't need to-the-minute
-- accuracy, just to not be wildly stale.
...
-- items only. `total_value` (what's actually displayed) stays in each
-- collector's own profile currency — only the ranking itself
-- (`total_value_usd`, via currency_rates_to_usd above) is converted, so
-- a collector whose currency isn't in the rate table yet still shows up
-- (coalesce to a 1:1 fallback) rather than silently vanishing from the
-- board.
...
-- ============================================================
-- Public read-only API / personal access tokens
-- (see api-tokens-migration.sql for the standalone version used when
-- updating an existing project)
-- ============================================================
...
-- Deliberately no update policy for normal users — see
-- api-tokens-migration.sql; the only writer of last_used_at is the
-- service-role client in lib/apiAuth.js.
...
-- ============================================================
-- Per-item reviews (separate from personal rating)
-- (see item-reviews-migration.sql for the standalone version used when
-- updating an existing project)
-- ============================================================
