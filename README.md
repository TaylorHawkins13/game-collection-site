...
4. If you're updating an **existing** project rather than starting fresh, also run `comics-migration.sql`, `currency-migration.sql`, `achievements-migration.sql`, `collectibles-migration.sql`, `region-migration.sql`, `copytype-migration.sql`, `ebayprice-migration.sql`, `completeness-migration.sql`, `fullycompleted-migration.sql`, `valuesnapshots-migration.sql`, `showcase-migration.sql`, and `activity-migration.sql` (same New query → paste → Run process). Brand new projects can skip these — `supabase-schema.sql` already includes everything.
