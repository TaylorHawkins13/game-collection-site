...
3. Trading cards, Books, Consoles, Vinyl, CDs, DVDs/Blu-rays, and VHS all have a "Search" auto-fill button too, and none of them need any setup — trading cards use the free Pokémon TCG/Scryfall (Magic) APIs, books use the free Open Library search API, vinyl/CD use the free MusicBrainz API, DVD/VHS use Apple's free iTunes Search API (fills in director, studio, and genre — same source for both, since it's the same movie either way), and consoles match against a built-in list of common systems (no external API for consoles at all). Comics and Funko Pops don't have auto-fill yet — see `ROADMAP.md`.
...
- **Loan tracker**: mark any owned item as "Loaned to ___" with a date, shown as a colored row on the card and filterable from the dashboard. Requires `loan-tracker-migration.sql` on existing projects.
