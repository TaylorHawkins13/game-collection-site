...
- **Accounts**: email/password signup & login (Supabase Auth), auto-creates a public profile with a chosen username. Includes a "Forgot password?" flow (`/forgot-password`, `/reset-password`) using Supabase's own reset-password email, and self-service account deletion with a 48-hour grace period (a "Danger zone" section in Profile Settings) — deletion sends a confirmation email so there's a real record of the request beyond the in-app banner.
